...
function Icon({ name }: { name: "spark" | "globe" | "desktop" | "tablet" | "phone" | "layers" | "eye" | "arrow" | "close" }) {
  const paths: Record<string, string> = {
    spark: "M12 2l1.7 6.3L20 10l-6.3 1.7L12 18l-1.7-6.3L4 10l6.3-1.7L12 2Zm6 14 .8 2.2L21 19l-2.2.8L18 22l-.8-2.2L15 19l2.2-.8L18 16Z",
    ...
    <div className="grain min-h-screen overflow-x-clip bg-ink text-paper">
      ...
      <main className="mx-auto max-w-7xl px-4 py-10 sm:px-6 sm:py-16 lg:px-8">
        <section className="mx-auto max-w-3xl text-center">
          <p className="mb-4 inline-flex items-center gap-2 rounded-full border border-ink-line bg-ink-2/60 px-3 py-1.5 font-mono text-[11px] uppercase tracking-widest text-amber"><Icon name="spark" /> Instant Web Revamp Engine</p>
        ...
        <section className="mt-12" aria-label="Interactive redesign workspace">
          ...
          <div className={`relative mx-auto overflow-hidden rounded-2xl border border-ink-line bg-ink-2 shadow-2xl transition-[max-width] duration-300 ${viewportClass}`}>
            {isAnalyzing && <div className="absolute inset-0 z-20 flex min-h-[480px] flex-col items-center justify-center bg-ink/95 p-6 text-center backdrop-blur-sm"><div className="flex h-14 w-14 items-center justify-center rounded-2xl border border-amber/40 bg-amber/10 text-amber"><Icon name="spark" /></div><h2 className="mt-5 font-display text-xl font-semibold">Building your preview</h2><p className="mt-2 font-mono text-xs text-amber">{ANALYSIS_STEPS[analysisStep]}</p><div className="mt-6 h-1.5 w-56 overflow-hidden rounded-full bg-ink-line"><div className="h-full rounded-full bg-amber transition-[width] duration-300" style={{ width: `${((analysisStep + 1) / ANALYSIS_STEPS.length) * 100}%` }} /></div></div>}
